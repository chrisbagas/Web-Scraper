...
def upload_to_dropbox(target_file_name, source_file, dropbox_token, dropbox_folder):
    ...
    if r.status_code != requests.codes.ok:
        print("Failed: upload file to Dropbox: {errcode} {content}".format(errcode=r.status_code, content=r.content))
        return None
    ...
    #if r.status_code != requests.codes.ok:
    #    print("Failed: get share link from Dropbox {errcode}  {content}".format(errcode=r.status_code, content=r.content))
    #    return None
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file.name', dest='file_name', help='file name that will be used as file name', required=True)
    ...
    temp = os.path.join(options.release_dir, options.file_name)
    file_url = upload_to_dropbox(options.file_name, temp, options.dropbox_token, options.dropbox_folder)
    if file_url == None: 
